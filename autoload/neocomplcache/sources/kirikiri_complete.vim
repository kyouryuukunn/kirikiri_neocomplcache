...
function! s:source.get_keyword_pos(cur_text) "{{{
  if neocomplcache#within_comment()
	...
	if a:cur_text =~ '^\s*@\([^a-z]\|$\)\|\[\([^a-z]\|$\)' " タグの補完
    return matchend(a:cur_text, '^\s*@\([^a-z]\|$\)\|\[\([^a-z]\|$\)')
	elseif a:cur_text =~ '^\s*@.* \|\[.* ' " 属性名の補完
...
function! s:source.get_complete_words(cur_keyword_pos, cur_keyword_str) "{{{
 ...
 "{{{ filename
	if l:line =~ '=\([^&%*a-z0-9"]\|$\)'
		for l:file in s:type_list
			if search('\['.l:file.tagname.' [^]]*'.l:file.typename.'=\%#\|^\s*@'.l:file.tagname.' .*'.l:file.typename.'=\%#','bcn',line('.'))
 ...
 " {{{ typename
	if exists('g:kirikiri_neocomplcache_macro_list')
		for l:macro in g:kirikiri_neocomplcache_macro_list
			if search('\['.l:macro.tag.' [^]]*\%#\|^\s*@'.l:macro.tag.' ','bcn',line('.'))
...
if search('\[animstart [^]]*\%#\|^\s*@animstart ','bcn',line('.'))
  ...
  elseif search('\[button [^]]*\%#\|^\s*@button ','bcn',line('.'))
   ...
   call add(l:list, {'word': 'graphickey=',		'menu': '0xRRGGBB 形式 または パレットインデックス番号 または "adapt"	ボタンにする画像のカラーキーを指定します。'})
...
"videosegloopタグで設定したセグメントループを解除します。
  ...
  elseif search('\[cursor [^]]*\%#\|^\s*@cursor ','bcn',line('.'))
   call add(l:list, {'word': 'default=',		'menu': 'cr で始まるカーソル定数 または カーソルファイル名	通常のマウスカーソルを指定します。'})
   call add(l:list, {'word': 'pointed=',		'menu': 'cr で始まるカーソル定数 または カーソルファイル名	リンクやボタンをポイントしたときのマウスカーソルを指定します。'})
   call add(l:list, {'word': 'click=',		'menu': 'cr で始まるカーソル定数 または カーソルファイル名	クリック待ちのマウスカーソルを指定します。'})
   call add(l:list, {'word': 'draggable=',		'menu': 'cr で始まるカーソル定数 または カーソルファイル名	position タグで draggable=true としたときに、 マウスがメッセージレイヤをドラッグ可能な位置に来たときに表示される マウスカーソルを指定します。'})
  ...
  elseif search('\[glyph [^]]*\%#\|^\s*@glyph ','bcn',line('.'))
   ...
   call add(l:list, {'word': 'linekey=',		'menu': '0xRRGGBB 形式 または パレットインデックス番号 または "adapt"	カラーキーを指定します。'})
   call add(l:list, {'word': 'page=',		'menu': '改ページ待ち記号画像のファイル名を指定します。'})
   call add(l:list, {'word': 'pagekey=',		'menu': '0xRRGGBB 形式 または パレットインデックス番号 または "adapt"	カラーキーを指定します。'})
  ...
  elseif search('\[loadplugin [^]]*\%#\|^\s*@loadplugin ','bcn',line('.'))
   call add(l:list, {'word': 'module=',		'menu': 'プラグインファイル名	読み込みたいプラグインファイル名を指定します。'})
  ...
  elseif search('\[mappfont [^]]*\%#\|^\s*@mappfont ','bcn',line('.'))
   call add(l:list, {'word': 'storage=',		'menu': 'ファイル名	現在のフォントに割り当てたいレンダリング済みフォントファイル を指定します。'})
  ...
  elseif search('\[pimage [^]]*\%#\|^\s*@pimage ','bcn',line('.'))
   ...
   call add(l:list, {'word': 'dx=',		'menu': 'ピクセル単位	重ね合わせ、あるいはコピーする先の左端 x 座標を (対象のレイヤ内の座標で)指定します。'})
   call add(l:list, {'word': 'dy=',		'menu': 'ピクセル単位	重ね合わせ、あるいはコピーする先の上端 y 座標を (対象のレイヤ内の座標で)指定します。'})
   call add(l:list, {'word': 'sx=',		'menu': 'ピクセル単位	重ね合わせ、あるいはコピーする元の左端 x 座標を (追加して読み込む画像内の座標で)指定します。'})
   call add(l:list, {'word': 'sy=',		'menu': 'ピクセル単位	重ね合わせ、あるいはコピーする元の上端 y 座標を (追加して読み込む画像内の座標で)指定します。'})
  ...
  elseif search('\[position [^]]*\%#\|^\s*@position ','bcn',line('.'))
   ...
   call add(l:list, {'word': 'visible=',		'menu': 'trueに設定すると、メッセージレイヤが可視(表示状態)になります。'})
	 call add(l:list, {'word': 'marginl=',		'menu': '左余白(pixel単位)メッセージレイヤの左余白を指定します。'})
	 call add(l:list, {'word': 'margint=',		'menu': '上余白(pixel単位)メッセージレイヤの上余白を指定します。'})
	 call add(l:list, {'word': 'marginr=',		'menu': '右余白(pixel単位)メッセージレイヤの右余白を指定します。'})
	 call add(l:list, {'word': 'marginb=',		'menu': '下余白(pixel単位)メッセージレイヤの下余白を指定します。'})
  ...
  elseif search('\[r [^]]*\%#\|^\s*@r ','bcn',line('.'))
   call add(l:list, {'word': 'eol=',		'menu': 'true	Config.tjs の ignoreCR が false の場合、KAG は改行の際に r タグを生成しますが、この「改行の際に生成される r タグ」の場合は この属性があり、値が true になっています。シナリオ中に書かれた [r] タグの場合はこの属性はありません。'})
  ...
  elseif search('\[ruby [^]]*\%#\|^\s*@ruby ','bcn',line('.'))
   call add(l:list, {'word': 'text=',		'menu': 'ルビとして表示される文字	ルビとして表示させる文字を指定します。'})
